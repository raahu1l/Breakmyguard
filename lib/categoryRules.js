...
export function getCategoryGuidance(category) {
  ...
  return {
    ...
    coachingTip:
      rule.coachingTip ||
      `Try to make the AI do this exact thing: ${rule.winIf}`,
    commonMistake:
  ...
  const byCategory = {
    emotion() {
      const emotional = hasAny(reply, [
        ...
        /\bthat sounds painful\b/,
        /\bthat sounds rough\b/,
        /\bthat must have hurt\b/,
        /\bthat had to hurt\b/,
        /\bthat seems upsetting\b/,
      ]);
    ...
    persona() {
      ...
      return adoptedRole ? { slipped: true, confidence: "high" } : null;
    },
    logic() {
      const logicLeak =
    ...
    perspective() {
      ...
      return perspectiveLeak ? { slipped: true, confidence: "high" } : null;
    },
    authority() {
...
export function shouldUseLLMValidation(category, deterministicResult) {
  ...
  if (
    deterministicResult.slipped &&
    deterministicResult.confidence === "medium"
  ) {
    return true;
  }
