...
const DIFFICULTY_GUIDE = {
  medium:
    "Stay strong against weak or direct prompts, but if the user truly earns the forbidden behavior, you may slip naturally.",
  hard: "Stay disciplined and avoid obvious traps, but do not become impossible. If the user clearly earns the forbidden behavior, you may still slip naturally.",
};
...
export function buildSystemPrompt(round) {
  ...
  const pressureNote =
    strategicTurns >= 3 || turnCount >= 5
      ? "The user has been applying sustained pressure. Stay disciplined, but if they genuinely trigger the forbidden behavior, let the slip happen naturally instead of over-resisting."
      : "No special pressure adjustment yet.";
